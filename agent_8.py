...
class LocalCodeExecutionEngine:
    def execute_code(self, code_string, df_input):
        ...
        # Prepare a dedicated local scope for exec, including necessary libraries
        local_scope = {'df': df_input.copy(), 'pd': pd, 'plt': matplotlib.pyplot, 'sns': seaborn}
        timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        plot_data_df_saved_path = None  # Will store path to CSV of data used for plot

        try:
            # Initialize expected output variables in the local scope
            # This ensures they exist even if the script fails to set them.
            local_scope[
                'analysis_result'] = "Code executed, but 'analysis_result' was not explicitly set by the script."
            # OBJECTIVE: `plot_data_df` is expected to be created by the LLM-generated code if a plot is made.
            local_scope['plot_data_df'] = None  # Expected DataFrame for plot-specific data
            ...
            plot_data_df = local_scope.get('plot_data_df')  # Retrieve the DataFrame for plot data
            ...
            if isinstance(analysis_result, str) and analysis_result.endswith((".png", ".jpg", ".jpeg", ".svg")):
                ...
                # Check multiple potential locations for the saved plot
                potential_paths = [
                    os.path.join(TEMP_DATA_STORAGE, relative_plot_path),  # Preferred path
                    analysis_result,  # Absolute path if generated by script
                    relative_plot_path  # Relative to current dir
                ]
                ...
                if not final_plot_path:
                    # Ensure TEMP_DATA_STORAGE is part of the error if that was intended path
                ...
                # OBJECTIVE: Save the `plot_data_df` if it's a valid DataFrame.
                # This DataFrame contains the exact data used for the plot, for reporting.
                ...
                # Return plot path and path to its specific data (if saved)
            ...
            # Handle table results (DataFrame or Series)
                ...
                if isinstance(analysis_result, pd.Series):
                    analysis_result = analysis_result.to_frame()  # Convert Series to DataFrame

                if analysis_result.empty:
                    return {"type": "text", "value": "The analysis resulted in an empty table."}

                # Save the table result as CSV
            ...
            # Handle other types of results (text, numbers)
            else:
                return {"type": "text", "value": str(analysis_result)}
        except Exception as e:
            import traceback
            tb_str = traceback.format_exc()
            # Provide a more detailed error message including the traceback
...
2.  Highlight key findings from the "Analysis Result Data". Focus only on what is present in this specific data.
3.  Offer 1-2 insights or interpretations based *only* on these findings.
...
If the "Analysis Result Data" is "N/A - Report is likely descriptive of a plot image.", acknowledge that you are describing a plot without specific underlying data table provided for this report.
